...
const ResourceSchema = z.object({
    ...
    resource_file: z.instanceof(File)
        .refine((file) => file.size === 0 || file.size <= MAX_FILE_SIZE, `Max file size is 50MB.`)
        .refine(
...
export async function upsertResource(prevState: ActionState, formData: FormData): Promise<ActionState> {
    ...
    if (resource_file && resource_file.size > 0) {
        ...
        // If we are editing (id exists) but no new file was uploaded,
        // we must fetch the existing resource to preserve its file URL and size.
        ...
        if (fetchError || !existingResource) {
            return { message: 'Could not find existing resource to update.', success: false };
        }
        download_url = existingResource.download_url;
        file_size = existingResource.file_size;
    } else {
        // If we are creating a new resource, a file is mandatory.
        return { 
            message: 'A file is required to create a new resource.', 
            success: false,
            errors: { resource_file: ['A file upload is required.'] } 
        };
    }
...
// ... deleteResource function remains the same ...
