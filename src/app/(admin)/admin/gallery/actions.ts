...
// RPC-style (no redirect) variants for client-driven navigation
export async function addGalleryImageRPC(formData: FormData): Promise<{ ok: boolean; error?: string; message?: string }> {
  const supabase = createSupabaseSSR()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return { ok: false, error: 'notauth' }
  const { data: roleData } = await supabase.from('user_roles').select('role').eq('user_id', user.id).maybeSingle()
  if (!roleData || (roleData.role !== 'admin' && roleData.role !== 'super_admin')) {
    return { ok: false, error: 'noperms' }
  }
  const { count } = await supabase.from('gallery_images').select('*', { count: 'exact', head: true })
  if ((count ?? 0) >= 10) return { ok: false, error: 'full' }

  const eventIdRaw = formData.get('event_id') as string | null
  const caption = (formData.get('caption') as string | null) || null
  const urlInput = (formData.get('image_url') as string | null) || null
  const file = formData.get('image') as File | null
  if (!file && !urlInput) return { ok: false, error: 'missing' }
  ...
  if (file) {
    ...
    const filename = `${user.id}/${Date.now()}.${ext}`
    const { error: uploadErr } = await supabase.storage.from('gallery').upload(filename, file, { contentType: file.type || 'image/jpeg', upsert: false })
    if (uploadErr) return { ok: false, error: 'upload' }
    storage_path = filename
    const { data } = supabase.storage.from('gallery').getPublicUrl(filename)
    image_url = data.publicUrl
  }
  ...
  const { error: insertErr } = await supabase.from('gallery_images').insert({ event_id, caption, storage_path, image_url, created_by: user.id })
  if (insertErr) return { ok: false, error: 'db' }
  revalidatePath('/admin/gallery'); revalidatePath('/'); revalidatePath('/gallery')
  return { ok: true, message: 'added' }
}

export async function replaceGalleryImageRPC(formData: FormData): Promise<{ ok: boolean; error?: string; message?: string }> {
  const supabase = createSupabaseSSR()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return { ok: false, error: 'notauth' }
  const { data: roleData } = await supabase.from('user_roles').select('role').eq('user_id', user.id).maybeSingle()
  if (!roleData || (roleData.role !== 'admin' && roleData.role !== 'super_admin')) return { ok: false, error: 'noperms' }

  const id = formData.get('id') as string | null
  const caption = (formData.get('caption') as string | null) || null
  const urlInput = (formData.get('image_url') as string | null) || null
  const file = formData.get('image') as File | null
  if (!id) return { ok: false, error: 'missingid' }
  if (!file && !urlInput && caption === null) return { ok: false, error: 'missing' }
  ...
  const { error: updateErr } = await supabase.from('gallery_images').update(updates).eq('id', id)
  if (updateErr) return { ok: false, error: 'db' }
  revalidatePath('/admin/gallery'); revalidatePath('/'); revalidatePath('/gallery')
